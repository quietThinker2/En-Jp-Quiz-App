...
class AllQuizActivity : AppCompatActivity(), OnPopupDismissListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Set Activity Title -> Number - Name
        ...
        val values = dbHelper.extractAllPrevious(segmentID)

        //User text area
        val editText: EditText = findViewById(R.id.editText)

        //first round translation prompt set to japanese
        val toTranslateTextView: TextView = findViewById(R.id.toTranslateTextView)
        toTranslateTextView.text = values[0]
        ...
        //Check Answer button trigger by pressing the Enter key on the keyboard
    ...
    private fun showPopUp(userText: String, segmentID: Int, values: ArrayList<String>, listener: OnPopupDismissListener){
        ...
        dialog.setOnDismissListener {
            val dbHelper = SQLiteHelper(this)
            val updatedList = dbHelper.extractAllPrevious(segmentID)
            //randomly switch language translation
            val rnds = (0..1).random()
            //set english
            if (rnds == 0) {
                val toTranslateTextView: TextView = findViewById(R.id.toTranslateTextView)
                //prevent hiragana and katakana english to japanese confusion
                if(updatedList[2] == ""){
